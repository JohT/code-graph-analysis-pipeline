    ...
    find "${directory}" -type f -name "*Visualization.sh" | sort | while read -r visualization_script_file; do
        ...
        if [ -n "${ANALYSIS_DOMAINS_TO_SKIP:-}" ]; then
            ...
            for domainToSkip in "${domainsToSkip[@]}"; do
                if [[ "${visualization_script_file}" == *"/${domainToSkip}/"* ]]; then
                    echo "${SCRIPT_NAME}: Skipping ${visualization_script_filename} (domain '${domainToSkip}' is in ANALYSIS_DOMAINS_TO_SKIP)."
                    skipThisScript=true
                    break
                fi
            done
