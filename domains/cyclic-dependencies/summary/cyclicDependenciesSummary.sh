...
# Creates a Markdown report summarising all cyclic dependency analysis results for Java packages, Java artifacts, TypeScript modules, and SCIP Semantic Index modules and artifacts.
...
assemble_cyclic_dependencies_report() {
    ...
    execute_limited_table \
        "${CYCLIC_DEPS_QUERY_CYPHER_DIR}/Cyclic_Dependencies_for_SCIP_Module.cypher" \
        "SCIP_Semantic_Index_Module/Cyclic_Dependencies_for_SCIP_Module.csv" \
        "${report_include_directory}/Cyclic_Dependencies_for_SCIP_Module.md"

    execute_limited_table \
        "${CYCLIC_DEPS_QUERY_CYPHER_DIR}/Cyclic_Dependencies_Breakdown_for_SCIP_Module.cypher" \
        "SCIP_Semantic_Index_Module/Cyclic_Dependencies_Breakdown_for_SCIP_Module.csv" \
        "${report_include_directory}/Cyclic_Dependencies_Breakdown_for_SCIP_Module.md"

    execute_limited_table \
        "${CYCLIC_DEPS_QUERY_CYPHER_DIR}/Cyclic_Dependencies_Breakdown_Backward_Only_for_SCIP_Module.cypher" \
        "SCIP_Semantic_Index_Module/Cyclic_Dependencies_Breakdown_Backward_Only_for_SCIP_Module.csv" \
        "${report_include_directory}/Cyclic_Dependencies_Breakdown_Backward_Only_for_SCIP_Module.md"

    execute_limited_table \
        "${CYCLIC_DEPS_QUERY_CYPHER_DIR}/Cyclic_Dependencies_between_SCIP_Artifacts_as_unwinded_List.cypher" \
        "SCIP_Semantic_Index_Artifact/Cyclic_Dependencies_between_SCIP_Artifacts_as_unwinded_List.csv" \
        "${report_include_directory}/Cyclic_Dependencies_between_SCIP_Artifacts.md"
    ...
    # -- Copy no-SCIP-data fallback template --------------------------------
    cp -f "${CYCLIC_DEPENDENCIES_SUMMARY_DIR}/report_no_scip_data.template.md" \
        "${report_include_directory}/report_no_scip_data.template.md"

    # -- Copy graph visualization references if available ------------------
    # These files are generated by cyclicDependenciesVisualization.sh when the Visualization report runs.
    # The "|| true" prevents failure when the visualization report has not run yet.
    cp -f "${FULL_REPORT_DIRECTORY}/Java_Package/Graph_Visualizations/GraphVisualizationsReferenceForSummary.md" \
        "${report_include_directory}/GraphVisualizationsJavaPackageReference.md" 2>/dev/null || true
    cp -f "${FULL_REPORT_DIRECTORY}/Typescript_Module/Graph_Visualizations/GraphVisualizationsReferenceForSummary.md" \
        "${report_include_directory}/GraphVisualizationsTypescriptModuleReference.md" 2>/dev/null || true
    cp -f "${FULL_REPORT_DIRECTORY}/SCIP_Semantic_Index_Module/Graph_Visualizations/GraphVisualizationsReferenceForSummary.md" \
        "${report_include_directory}/GraphVisualizationsScipModuleReference.md" 2>/dev/null || true
