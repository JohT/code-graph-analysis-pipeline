...
anomaly_detection_pipeline() {
    ...
    time execute_cypher_queries_until_results "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-Betweenness-Exists.cypher" "${@}" \
                                              "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-Betweenness-Write.cypher" "${@}" >/dev/null
    # Query Feature: Determine the local clustering coefficient if not already done
    time execute_cypher_queries_until_results "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-LocalClusteringCoefficient-Exists.cypher" "${@}" \
                                              "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-LocalClusteringCoefficient-Write.cypher" "${@}" >/dev/null
    # Query Feature: Determine the page rank if not already done
    time execute_cypher_queries_until_results "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-PageRank-Exists.cypher" "${@}" \
                                              "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-PageRank-Write.cypher" "${@}" >/dev/null
    # Query Feature: Determine the article rank if not already done
    time execute_cypher_queries_until_results "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-ArticleRank-Exists.cypher" "${@}" \
                                              "${ANOMALY_DETECTION_SCRIPT_DIR}/AnomalyDetectionFeature-ArticleRank-Write.cypher" "${@}" >/dev/null
