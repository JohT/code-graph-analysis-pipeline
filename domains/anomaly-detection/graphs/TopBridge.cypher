...
OPTIONAL MATCH (source)-[dependency:DEPENDS_ON]->(central)
  ...
  LIMIT 40
   ...
   WITH *, "label = \"" + sourceNameSplit + "\"; "                              AS directInLabel
   WITH *, " [" + directInLabel + "]; "                                         AS directInNodeProperties
...
OPTIONAL MATCH (source)<-[dependency:DEPENDS_ON]-(central)
  ...
  LIMIT 40
   ...
   // Split long names like inner classes identified by a dollar sign ($)
   WITH *, replace(source.name, '$', '$\\n')                                    AS sourceNameSplit
   WITH *, "label = \"" + sourceNameSplit + "\"; "                              AS directOutLabel
   WITH *, " [" + directOutLabel + directOutColor + "]; "                       AS directOutNodeProperties
