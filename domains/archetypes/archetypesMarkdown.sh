...
# It is designed as an entry point and delegates the execution to the dedicated "archetypesSummary.sh" script that does the "heavy lifting".

# Note that "scripts/prepareAnalysis.sh" is required to run prior to this script.

# Requires archetypesSummary.sh
...
ARCHETYPES_SUMMARY_DIR=${ARCHETYPES_SUMMARY_DIR:-"${ARCHETYPES_SCRIPT_DIR}/summary"} # Contains everything (scripts, queries, templates) to create the Markdown summary report for archetypes

# Delegate the execution to the responsible script.
source "${ARCHETYPES_SUMMARY_DIR}/archetypesSummary.sh"
