// Archetypes Labels: Label code units of archetype "Hub" by looking for the (at most) top 20 entries with the highest degree >= 80% percentile and a local clustering coefficient <= 10% percentile. Requires features/*.cypher to be run first.
// Shows code with many connections that are not well integrated into a cluster.
   ...
   WHERE $projection_node_label IN labels(codeUnit)
    ...
    WITH collect(codeUnit)                                                  AS codeUnits
        ,percentileDisc(degree, 0.80)                                       AS degreeThreshold
