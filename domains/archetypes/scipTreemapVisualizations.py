...
# This Python script uses Plotly Treemap Charts (https://plotly.com/python/treemaps) to visualize structural archetype classification results for SCIP-indexed code.
...
def parse_input_parameters() -> Parameters:
    ...
    parser = argparse.ArgumentParser(
        description="Treemap visualization for structural archetypes (Authority, Bottleneck, Hub) for SCIP-indexed code across the file structure.")
...
def get_file_path(name: str, parameters: Parameters, extension: str = 'svg') -> str:
    ...
    if parameters.is_verbose():
        print(f"scipTreemapVisualizations: Saving file {name}")
    return name
...
def query_data() -> pd.DataFrame:
    """Query SemanticCodeIndexInternalType nodes with archetype ranks, using the file property for treemap layout."""
    query: LiteralString = """
        MATCH (stats:SemanticCodeIndexInternalType)
        WHERE stats.archetypeAuthorityRank  IS NOT NULL
           OR stats.archetypeBottleneckRank IS NOT NULL
           OR stats.archetypeHubRank        IS NOT NULL
         WITH max(stats.archetypeAuthorityRank)  AS maxAuthorityRank
             ,max(stats.archetypeBottleneckRank) AS maxBottleneckRank
             ,max(stats.archetypeHubRank)        AS maxHubRank
        MATCH (n:SemanticCodeIndexInternalType)
        WHERE n.archetypeAuthorityRank  IS NOT NULL
           OR n.archetypeBottleneckRank IS NOT NULL
           OR n.archetypeHubRank        IS NOT NULL
        OPTIONAL MATCH (a:SemanticCodeIndexArtifact)-[:CONTAINS]->(n)
          WITH *
              ,coalesce(a.projectName + '/', '') AS projectName
        RETURN replace(projectName + n.file, '//', '/') AS filePath
              ,coalesce(toFloat(n.archetypeAuthorityRank)  / maxAuthorityRank,  0) AS normalizedAuthorityRank
              ,coalesce(toFloat(n.archetypeBottleneckRank) / maxBottleneckRank, 0) AS normalizedBottleneckRank
              ,coalesce(toFloat(n.archetypeHubRank)        / maxHubRank,        0) AS normalizedHubRank
...
print(f"scipTreemapVisualizations: Querying {title_prefix} archetype data for treemap visualization...")
archetype_file_paths = query_data()

if archetype_file_paths.empty:
    print(f"scipTreemapVisualizations: No archetype data found for {title_prefix}. Skipping treemap visualizations.")
    driver.close()
    exit(0)

print(f"scipTreemapVisualizations: Preparing {title_prefix} archetype data for treemap visualization...")
archetype_file_paths = prepare_data_for_treemap(archetype_file_paths)

# --- Visualizing Archetypes Overview

print(f"scipTreemapVisualizations: Creating {title_prefix} archetypes overview treemap visualization...")
...
def plot_single_archetype_treemap(archetype: Archetypes, title_prefix: str, file_index: int, data: pd.DataFrame):
    print(f"scipTreemapVisualizations: Creating {title_prefix} archetype '{archetype}' treemap visualization...")
...
driver.close()
print("scipTreemapVisualizations: Successfully created treemap visualizations.")
