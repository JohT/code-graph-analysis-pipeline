...
# Executes selected archetypes Cypher queries for GraphViz visualization.
# Visualizes top ranked archetypes (Authority, Bottleneck, Hub).
# Requires an already running Neo4j graph database with already scanned and analyzed artifacts.
# The reports (csv, dot and svg files) will be written into the sub directory reports/archetypes/{language}_{codeUnit}.
...
## Get this "domains/archetypes/graphs" directory if not already set
# Even if $BASH_SOURCE is made for Bourne-like shells it is also supported by others and therefore here the preferred solution. 
# CDPATH reduces the scope of the cd command to potentially prevent unintended directory changes.
# This way non-standard tools like readlink aren't needed.
ARCHETYPES_GRAPHS_DIR=${ARCHETYPES_GRAPHS_DIR:-$( CDPATH=. cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P )}
#echo "archetypesGraphs: ARCHETYPES_GRAPHS_DIR=${ARCHETYPES_GRAPHS_DIR}"

# Get the "scripts" directory by taking the path of this script and going one directory up.
SCRIPTS_DIR=${SCRIPTS_DIR:-"${ARCHETYPES_GRAPHS_DIR}/../../../scripts"} # Repository directory containing the shell scripts

# Get the "cypher" query directory for gathering features.
ARCHETYPES_FEATURE_CYPHER_DIR=${ARCHETYPES_FEATURE_CYPHER_DIR:-"${ARCHETYPES_GRAPHS_DIR}/../features"}
...
ARCHETYPES_TOP_N_GRAPHS=${ARCHETYPES_TOP_N_GRAPHS:-5} # Number of top ranked graphs to visualize per query for archetypes.

# Define functions to execute cypher queries from within a given file
source "${SCRIPTS_DIR}/executeQueryFunctions.sh"

# Ensure PageToArticleRank feature is available (required for Authority visualization).
# 
# Required Parameters:
# - projection_name=...
...
set_required_features() {
    ...
    echo "archetypesGraphs: $(date +'%Y-%m-%dT%H:%M:%S%z') Collecting features for ${nodeLabel} nodes..."
...
update_markdown_references() {
    ...
    if [ ! -f "${markdown_graph_visualizations_reference_file}" ]; then
        echo "archetypesGraphs: $(date +'%Y-%m-%dT%H:%M:%S%z') Creating ${markdown_graph_visualizations_reference_file}..."
        {
...
# Outputs (at most) ARCHETYPES_TOP_N_GRAPHS indexed files (for report_name="TopHub" then TopHub1, TopHub2,...) with a focused visualization.
# 
# Required Parameters:
# - report_name=...
#   Name of the query and then also the resulting visualization file.
...
create_graph_visualization() {
    ...
    echo "archetypesGraphs: $(date +'%Y-%m-%dT%H:%M:%S%z') Creating ${language} ${nodeLabel} ${report_name} visualizations with template ${template_name}..."
    ...
    for ((index=1; index<=ARCHETYPES_TOP_N_GRAPHS; index++)); do
        # Query Graph data
        local resultFileName="${detail_report_directory}/${report_name}${index}"
        local queryResultFile="${resultFileName}.csv"
        execute_cypher "${ARCHETYPES_GRAPHS_DIR}/${report_name}.cypher" "${@}" "projection_node_rank=${index}" > "${queryResultFile}" || true
        ...
        source "${VISUALIZATION_SCRIPTS_DIR}/visualizeQueryResults.sh" "${queryResultFile}" --template "${ARCHETYPES_GRAPHS_DIR}/${template_name}.template.gv"
...
# Run queries, output their results in GraphViz format and create Graph visualizations.
#
# Required Parameters:
# - projection_language=...
#   Name of the associated programming language. Examples: "Java", "Typescript"
...
# - projection_weight_property=...
#   Name of the node property that contains the dependency weight. Example: "weight"
archetypes_graph_visualization() {
    set_required_features "${@}"
...
archetypes_graph_visualization "${QUERY_NODE}=Artifact" "${QUERY_LANGUAGE}=Java" "${QUERY_WEIGHT}=weight"
...
echo "archetypesGraphs: $(date +'%Y-%m-%dT%H:%M:%S%z') Successfully finished."
