...
GraphSAGE query files are present in [`queries/node-embeddings/`](./queries/node-embeddings/) but are **disabled by default** in `nodeEmbeddingsCsv.sh` (commented out in `GRAPHSAGE_ENABLED` flag at line 46). GraphSAGE requires feature properties (e.g., `degreeForNodeEmbeddings` from Node_Embeddings_0b_Prepare_Degree.cypher) and incurs significantly higher compute cost due to model training. The implementation uses `gds.beta.graphSage.train()` with node feature properties only; it does not require node labels. The files are retained for future use when computational resources are available.
