...
def apply_umap(embeddings_data: pd.DataFrame, verbose: bool) -> pd.DataFrame:
    ...
    Args:
        embeddings_data: DataFrame with an 'embedding' column of float arrays (must have >= 2 rows).
...
def _generate_charts_for_label(
    ...
    for embedding_property, algorithm_name in EMBEDDING_PROPERTIES.items():
        ...
        # Skip chart generation for datasets with fewer than 2 points (cannot run UMAP or compute centrality)
        if len(embeddings_data) < 2:
            if verbose:
                print(f"{SCRIPT_NAME}: Skipping chart for {node_label} {algorithm_name} — insufficient data ({len(embeddings_data)} point{'s' if len(embeddings_data) != 1 else ''})")
            all_reduced_for_label.append((None, algorithm_name))
            continue
