...
successful() {
  ...
  # If this script was sourced (by runTests.sh) use `return` so the runner
  # continues; if executed directly, use `exit` to terminate the process.
...
fail() {
  ...
  # Keep temporary test directory for inspection and fail fast so CI detects the failure.
  if [ -n "${temporaryTestDirectory}" ]; then
    echo -e "${COLOR_DE_EMPHASIZED}${SCRIPT_NAME}: Temporary test directory preserved for debugging: ${temporaryTestDirectory}${COLOR_DEFAULT}"
  fi
  return 1
}
...
NEO4J_INSTALLATION_NAME="neo4j-community-2026.07.0"
