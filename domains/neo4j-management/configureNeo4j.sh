...
append_configuration_from_template() {
    ...
    if grep -q "^${EYE_CATCHER_COMMENT_PREFIX}" "${NEO4J_CONFIG}"; then
        ...
        rm -f "${NEO4J_CONFIG}.edit.backup" # The backup is created even if not needed to be compatible with both GNU sed and BSD sed (e.g. on macOS) when using -i, but it is removed afterwards to avoid leaving unnecessary files.
    fi
...
if [ "${NEO4J_MAJOR_VERSION_NUMBER}" -ge 5 ]; then
    echo "${SCRIPT_NAME}: Neo4j v5 or higher detected"
    {
        echo ""
    ...
    echo "${SCRIPT_NAME}: Neo4j v4 or lower detected"
    {
        echo ""
