...
source "${NEO4J_MANAGEMENT_DIR}/waitForNeo4jHttpFunctions.sh"
...
if [ "${isDatabaseReady}" = "false" ]; then
    ...
    if ! isWindows; then
        echo "stopNeo4j: ${neo4j_directory} still running. Something went wrong. Details see 'NEO4J_HOME=${NEO4J_DIR} ${NEO4J_BIN}/neo4j status'."
        exit 1
    fi
fi
